"""갑옷+방패 시트 (4×2) → 상단 4 갑옷은 Icons/Equipment/, 하단 4 방패는 Icons/Unused/Shields/."""
from collections import deque
from pathlib import Path
from PIL import Image

SCRIPT_DIR = Path(__file__).resolve().parent
SOURCE_SHEET = SCRIPT_DIR / "source_armor_with_unused_shields_2026_05_13.png"
EQUIP_DIR = SCRIPT_DIR.parents[1] / "Icons" / "Equipment"
SHIELD_DIR = SCRIPT_DIR.parents[1] / "Icons" / "Unused" / "Shields"
# (이름, 출력 디렉터리)
TARGETS = [
    ("chainmail_armor",   EQUIP_DIR),
    ("knight_plate_armor",EQUIP_DIR),
    ("ranger_vest",       EQUIP_DIR),
    ("mystic_robe",       EQUIP_DIR),
    ("iron_shield",       SHIELD_DIR),
    ("tower_shield",      SHIELD_DIR),
    ("ranger_buckler",    SHIELD_DIR),
    ("royal_guard_shield",SHIELD_DIR),
]
...
def main():
    ...
    for i,(name,outd) in enumerate(TARGETS):
        ...
        x0 = max(0, col*cw - LEFT_PAD)
        x1 = min(sw, (col+1)*cw + RIGHT_PAD)
