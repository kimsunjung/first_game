...
> 상태 메모 (2026-04-15)
> 이 문서는 구현 당시의 계획서다. 현재 프로젝트는 `main.tscn` 단일 구조가 아니라 `Scenes/Maps/town.tscn`, `field_1.tscn`, `field_2.tscn`, `dungeon_1.tscn`, `dungeon_2.tscn`로 분리되어 있다.
> 문서 안의 `main.tscn` 참조는 현재 맵 씬 구조로 치환해서 읽어야 한다.
